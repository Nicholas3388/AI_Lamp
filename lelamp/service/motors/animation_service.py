...
class AnimationService:
    def __init__(self, port: str, lamp_id: str, fps: int = 30, duration: float = 5.0, idle_recording: str = "idle"):
        ...
        # 新增：执行次数控制
        self._repeat_count: int = 1          # 总执行次数（默认1次）
        self._current_repeat: int = 0        # 当前已执行次数
    ...
    def _handle_play(self, payload: Union[str, Tuple[str, int]]):
        """Start playing a recording with interpolation from current state"""
        if not self.robot:
            print("Robot not connected")
            return
        
        # 解析payload，分离动作名和执行次数
        if isinstance(payload, tuple):
            recording_name, repeat_count = payload
            # 校验执行次数（必须是正整数）
            if not isinstance(repeat_count, int) or repeat_count < 1:
                print(f"无效的执行次数 {repeat_count}，默认执行1次")
                repeat_count = 1
        else:
        ...
        print(f"开始播放 {recording_name} (执行{repeat_count}次)，启用插值过渡")
        
        # 重置播放状态
        self._current_recording = recording_name
        self._current_actions = actions
        self._current_frame_index = 0
        self._repeat_count = repeat_count    # 设置总执行次数
        self._current_repeat = 0             # 重置已执行次数
    ...
    def _continue_playback(self):
        ...
        try:
            ...
            if self._current_frame_index < len(self._current_actions):
                ...
                print(f"{self._current_recording} 已执行 {self._current_repeat}/{self._repeat_count} 次")

                # 判断是否需要继续重复
                if self._current_repeat < self._repeat_count:
                    # 未达到指定次数：重置帧索引，重新播放（可选择是否保留插值）
                    self._current_frame_index = 0
                    # 可选：重复播放时是否重新插值到第一帧（根据需求决定）
                    # self._interpolation_frames = int(self.duration * 0.5 * self.fps)  # 短插值
                    # self._interpolation_target = self._current_actions[0]
                else:
                    # 达到指定次数：判断是否切回idle
                    if self._current_recording != self.idle_recording:
                        ...
                        if idle_actions is not None and len(idle_actions) > 0:
                            ...
                            self._current_frame_index = 0
                            self._repeat_count = 1          # idle动作默认执行1次（循环）
                            self._current_repeat = 0
                            if self._current_state is not None:
                                self._interpolation_frames = int(self.duration * self.fps)
                                self._interpolation_target = idle_actions[0]
                    else:
                        # 如果是idle动作，默认循环播放（重置帧索引）
                        self._current_frame_index = 0
